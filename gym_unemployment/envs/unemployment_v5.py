...
class UnemploymentLargeEnv_v5(gym.Env):
    ...
    def setup_couples(self,age: int,wage: float,benefitbasis: float,kansanelake: float,tyoelake: float,
        employment_state: int,time_in_state: float,used_unemp_benefit: float,
            ...
            puoliso_palkka: float,puoliso_benefitbasis: float,puoliso_kansanelake: float,puoliso_tyoelake: float,
            puoliso_tila: int,puoliso_time_in_state: float,puoliso_used_unemp_benefit: float,
...
#############################
    ...
    def update_wage_reduction_baseline(self,state: int,wage_reduction: float,pinkslip: int,time_in_state: float,g: int,initial_reduction: bool=False):
        '''
    ...
    def update_wage_reduction_sigma(self,state: int,wage_reduction: float,pinkslip: int,time_in_state: float,
            g: int,initial_reduction: bool=False):
        '''
        Pidetään kirjaa siitä, kuinka paljon palkkaa alennetaan työttämyyden keston suhteen,
        ja miten siitä palaudutaan
...
#############################
    ...
    def step(self, action, dynprog: bool=False, debug: bool=False):
        ...
        main_empstate,g,main_pension,main_old_wage,age,time_in_state,main_paid_pension,pinkslip,toe,\
            ...
            puoliso_tyoelake_maksussa,main_next_wage,\
            main_paid_wage,spouse_paid_wage,\
            pt_act,sp_pt_act,\
            main_basis_wage,spouse_basis_wage\
        ...
        if self.randomness:
            ...
            if sattuma[0]<move_prob:
                ...
                s1=s2+dis #self.disability_intensity[intage,g,main_empstate]
                        ...
                        if g>2: # naiset
                            if main_empstate not in set([3]): # entä 14 ??
                                ...
                                if sattuma[4]<0.39 and spouse_empstate not in set([3]): # orig 0.5
                                    ...
                                    puoliso_pinkslip=0
                                    spouse_moved=True
                                    puoliso_wage_reduction=self.update_wage_reduction(spouse_empstate,puoliso_wage_reduction,puoliso_pinkslip,puoliso_time_in_state,g)
                        else: # miehet
            ...
            if sattuma2[0]<move_prob and not spouse_moved and spouse_empstate!=15:
                ...
                if sattuma2[2]<s1/move_prob: # age<self.min_retirementage and 
                    ...
                    if spouse_empstate in set([0,1,4,10,12,13]) and age<self.min_retirementage: # not in set([2,3,5,6,7,8,9,11,12,15]):
                        ...
                        spouse_empstate,puoliso_kansanelake,puoliso_tyoelake_maksussa,puoliso_pension,spouse_wage,puoliso_time_in_state,puoliso_ove_paid,puoliso_basis_wage\
                            =self.move_to_retirement(puoliso_pension,spouse_wage,age,puoliso_kansanelake,puoliso_tyoelake_maksussa,
            ...
            if sattuma[3]<self.mort_intensity[intage,g] and self.include_mort and main_empstate!=15: 
                if puoliso>0: # avo- tai avioliitossa
                    ...
                    puoliso_tyoelake_maksussa=self.comp_familypension(puoliso,main_empstate,spouse_empstate,
                        tyoelake_maksussa,main_pension,age,puoliso_tyoelake_maksussa,
        ...
        if age>=self.max_retirementage and spouse_empstate not in set([2,3,15]):
            spouse_empstate,puoliso_kansanelake,puoliso_tyoelake_maksussa,puoliso_pension,spouse_paid_wage,puoliso_time_in_state,puoliso_ove_paid,spouse_basis_wage\
                =self.move_to_retirement(puoliso_pension,spouse_wage,age,puoliso_kansanelake,puoliso_tyoelake_maksussa,spouse_empstate,\
        ...
        if (not moved) and main_empstate != 15:
            ...
            is_spouse=False
            main_empstate,kansanelake,tyoelake_maksussa,main_pension,main_paid_wage,time_in_state,pinkslip,\
        ...
        if not done:
            ...
            netto,benq,netto_omat,netto_puoliso=self.get_benefits(main_empstate,main_paid_wage,kansanelake,tyoelake_maksussa,main_pension,time_in_state,pinkslip,
                ...
                puoliso_time_in_state,puoliso_unempwage,puoliso_unempwage_basis,puoliso_used_unemp_benefit,0,0,g,spouse_g,potential_wage=pot_main_wage,potential_spouse_wage=pot_spouse_wage)
...
#  Perussetti, tuottaa korkean elastisuuden
    ...
    def log_utility_mort_ove_params(self):
        ...
        self.salary_const_svpaiva=np.array([0.20,0.14,0.085,0.15,0.08,0.04])*self.timestep # pitkällä svpäivärahalla muutos nopeaa fyysisissä töissä
        ...
        self.men_mu_scale_kokoaika=0.035 #11 #250 #120 #0.075 # 0.075 #18 # 0.14 # 0.30 # 0.16 # how much penalty is associated with work increase with age after mu_age
        self.men_mu_scale_osaaika=0.021 #09 #14 #040 #0.075 # 0.075 #18 # 0.14 # 0.30 # 0.16 # how much penalty is associated with work increase with age after mu_age
        self.men_mu_age=self.min_retirementage#-1.0
        self.men_kappa_hoitovapaa=0.080 # hyäty hoitovapaalla olosta
        ...
        self.men_kappa_param=np.array([-0.455, -0.455, -0.490, -0.655, -0.950, -1.435]) # osa-aika 10h, 20h, 30h, kokoaika 40h, 50h, 60h
        ...
        self.women_mu_scale_osaaika=0.013 #09 #14 #040 #0.075 # 0.075 # 0how much penalty is associated with work increase with age after mu_age
        self.women_mu_age=self.min_retirementage
        self.women_kappa_hoitovapaa=0.350 # 0.27
        self.women_kappa_ve=0.27
        self.women_kappa_pinkslip_young=0.35
        self.women_kappa_pinkslip_middle=0.20
        self.women_kappa_pinkslip_elderly=0.25
        self.women_kappa_param=np.array([-0.253, -0.240, -0.280, -0.375, -0.780, -1.100]) # osa-aika 10h, 20h, 30h, kokoaika 40h, 50h, 60h
    ...
    def log_utility(self,income: float,employment_state: int,age,g=0,pinkslip=0,prefnoise=0,spouse=0,debug=False,pt_factor=0):
        ...
        if self.include_preferencenoise:
            # normaali
            u=math.log(prefnoise*income/self.inflationfactor)+kappa
            equ=(income/self.inflationfactor)*math.exp(kappa)
        else:
            u=math.log(income/self.inflationfactor)+kappa
            equ=(income/self.inflationfactor)*math.exp(kappa)
